<?php
// $Id: mambo.php,v 1.98 2004/04/07 12:47:51 saka_car Exp $
/**
* Foundation classes
...
class mosMainFrame {
	...
	*/
	...
	function _setConfig( $basePath='.' ) {
		...
		$this->_config->editor = $mosConfig_editor;
		...
		$this->_config->emailpass = $mosConfig_emailpass;
	...
	function _setAdminPaths( $option, $basePath='.' ) {
		...
		switch ($prefix) {
			...
			if (file_exists( "$basePath/components/com_$option/$option.php" )) {
				...
				$this->_path->front_html = "$basePath/components/com_$option/$option.html.php";
			}
			break;
		}
	}
...
class mosContent extends mosDBTable {
	...
	*/
	function search( $text, $state='1', $sectionPrefix='' ) {
		...
		$this->_db->setQuery( "SELECT a.title, a.created AS created,"
		. "\n	a.introtext AS text,"
	...
	*/
	...
	function check()
	{
		...
		if (eregi( "[^0-9A-Za-z]", $this->username)) {
			$this->_error = sprintf( _VALID_AZ09, "login name.",8 );
			return false;
		}
