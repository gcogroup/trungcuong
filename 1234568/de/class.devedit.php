<?php 
	...
	function DisplayIncludes ($file, $errorMsg)
	{
		...
		if($DevEditPath_Full == "")
		{
			$filePath = @$_GET["DEP"] . "/de_includes/$file";
			$thePath = @$_GET["DEP"];
		}
		else
		{
		...
		include_once($thePath . "/de_lang/language.php");
	...
	class DevEdit
	{
		...
		function isIE55OrAbove()
		{
			...
			$browserCheck3 = ( is_numeric(strpos($_SERVER["HTTP_USER_AGENT"], "MSIE 5.5")) || is_numeric(strpos($_SERVER["HTTP_USER_AGENT"], "MSIE 6")) ) ? true : false;
		...
		function ShowControl($Width, $Height, $ImagePath)
		{
			...
			//sua them - dua thu muc chua flash trung voi thu muc chua image
			$this->__flashPath = $ImagePath;
			...
			else
			{
					...
					<iframe onBlur="updateValue()" id=previewFrame height=80% style="width=100%; display:none" style="border: 1px solid #D4D0C8"></iframe>
        				...
        				<script language="JavaScript">
        					...
        					function setValue()
							{
									foo.document.write('<?php echo str_replace("<", "&lt;", eregi_replace("</script>", "<\\/script>", $this->__initialValue)); ?>');
									foo.document.close();

									fooStyles.document.write('<?php echo str_replace("<", "&lt;", eregi_replace("</script>", "<\\/script>", $this->__initialValueNoBase)); ?>');
