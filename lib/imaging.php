...
function create_thumb($filepath, $filename)
{
    ...
    $normal = $filepath . 'normal_' . $filename;
    $thumb = $filepath . 'thumb_' . $filename;
    $imagesize = getimagesize($image);
    if (!file_exists($thumb)) {
        if (!resize_image($image, $thumb, $CONFIG['auto_thumbnail_dimension'], 'gd2', 'wd', $CONFIG['watermark_text']))
		{
            return false;
		}
		else
    ...
    if (max($imagesize[0], $imagesize[1]) > $CONFIG['max_image_width'] && !file_exists($normal)) {
        if (!resize_image($image, $normal, $CONFIG['max_thumb_width'], 'gd2', 'wd', $CONFIG['watermark_text']))
		{
            return false;
		}
		else
...
function create_thumb_url($file_url)
{
	...
	$new_file_name=get_new_file_name($file_name,"remote_");
	
	$filepath=$CONFIG['root_path'].$CONFIG['upload_image_path'];
	$image=$file_url;
    $normal = $filepath . 'normal_' . $new_file_name;
    $thumb = $filepath . 'thumb_' . $new_file_name;
    ...
    if (!file_exists($thumb)) {
		...
		{
			$thumbnail['thumb']='thumb_' . $new_file_name;
		}
    }

    if (max($imagesize[0], $imagesize[1]) > $CONFIG['max_image_width'] && !file_exists($normal)) {
        if (!resize_image($image, $normal, $CONFIG['max_image_width'], 'gd2', 'wd', $CONFIG['watermark_text']))
		{
            return false;
		}
		else
		{
			$thumbnail['normal']='normal_' . $new_file_name;
		}
    }
