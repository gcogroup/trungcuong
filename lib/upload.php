...
class Upload {
  ...
  function Upload($upload_path,$allowed_mediatypes='',$upload_mode=2,$max_size=6000,$max_image_width=1000024,$max_image_height=1000024) {
    global $config;
  ...
  function check_max_filesize() {
    if ($this->_FILES[$this->field_name]['size'] > $this->max_size) {
      return false;
    }
    else {
      return true;
...
*/
  function upload_file($field_name, $check_image=0, $file_name = "") {
    ...
    if ($file_name != "") {
      ereg("(.+)\.(.+)", $file_name, $regs);
      $this->name = $regs[1];
      ereg("(.+)\.(.+)", $this->_FILES[$this->field_name]['name'], $regs);
